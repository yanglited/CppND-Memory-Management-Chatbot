    ...
    if(file)
    {
        ...
        while(getline(file, lineStr))
        {
            ...
            if(type != tokens.end())
            {
                // check for id
                auto idToken =
                    std::find_if(tokens.begin(), tokens.end(), [](const std::pair<std::string, std::string>& pair) {
                ...
                if(idToken != tokens.end())
                {
                    ...
                    if(type->second == "NODE")
                    {
                        ...
                        auto newNode =
                            std::find_if(_nodes.begin(), _nodes.end(), [&id](std::unique_ptr<GraphNode> const& node) {
                    ...
                    if(type->second == "EDGE")
                    {
                        ...
                        auto parentToken = std::find_if(
                            tokens.begin(), tokens.end(), [](const std::pair<std::string, std::string>& pair) {
                        ...
                        if(parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            auto parentNode = std::find_if(
                                _nodes.begin(), _nodes.end(), [&parentToken](std::unique_ptr<GraphNode> const& node) {
                            ...
                            // create new edge
                            auto edge = std::make_unique<GraphEdge>(GraphEdge(id));
                            ...
                            // Ownership of the edge belongs to the parent node, and the child node should have the
                            // information as well:
